...
export async function main(ns) {
  const refreshDelay = 600000, batchDelay = 200, secBuffer = 0.05;
  let firstRun = true;
  const scanAll = (host = "home", found = new Set()) => { found.add(host); for (const n of ns.scan(host)) if (!found.has(n)) scanAll(n, found); return [...found]; };

  while (true) {
    if (firstRun) { firstRun = false; await ns.run("money.js"); }

    const playerHack = ns.getHackingLevel();
    const servers = scanAll().filter(s => ns.hasRootAccess(s) && ns.getServerMaxMoney(s) > 0 && ns.getServerRequiredHackingLevel(s) <= playerHack);
    ...
    const hosts = scanAll().filter(h => ns.hasRootAccess(h)), ramPerThread = ns.getScriptRam("worker.js");
    const leftovers = []; let hostCount = 0;
    ...
    for (const { host, threads } of leftovers) {
      ...
      const parts = []; if (hack) parts.push(`hacking:${hack}`);
